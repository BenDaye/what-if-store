...
import { formatListRequest, formatListResponse, handleServerError, write } from '@what-if-store/utils';
  ...
  upload: protectedUserProcedure
    ...
    .mutation(async ({ ctx: { session, prisma }, input }) => {
      ...
      await prisma.$transaction(async (tx) => {
        ...
        if (existedFile) {
          if (existedFile.Users.findIndex((user) => user.id === session.user.id) > -1) return;
