...
import { DelayedError, MetricsTime, Queue, UnrecoverableError, Worker } from 'bullmq';
import type { z } from 'zod';
import type { ApplicationStatus } from '@what-if-store/prisma';
import { AuthRole } from '@what-if-store/prisma';
...
import { validStatusTransition } from './utils/validator';

const applicationChangeStatusJobDataType = applicationChangeStatusInputSchema.extend({
  userId: idSchema,
  reviewerId: idSchema.optional(),
});
type ApplicationChangeStatusJobDataType = z.infer<typeof applicationChangeStatusJobDataType>;

export type ApplicationChangeStatusQueue = Queue<ApplicationChangeStatusJobDataType, ApplicationStatus>;

export type ApplicationChangeStatusWorker = Worker<ApplicationChangeStatusJobDataType, ApplicationStatus>;
...
export const applicationChangeStatusProcessor: ApplicationChangeStatusProcessor = async (job, token) => {
  const { id: applicationId, userId: applicantId, reviewerId, status: nextStatus } = job.data;

  return await prisma.$transaction(async (_prisma): Promise<ApplicationStatus> => {
    ...
    const [isValidStatusTransition, isReviewRequired] = validStatusTransition(
      currentStatus,
      nextStatus,
      applicant.role,
    );
    if (!isValidStatusTransition) throw new UnrecoverableError('Invalid status transition');

    if (applicant.role === AuthRole.Provider) {
      if (application.providerId !== applicantId) throw new UnrecoverableError('Invalid applicant');
      ...
      if (isReviewRequired) {
        ...
        if (reviewer.role !== AuthRole.Admin) throw new UnrecoverableError('Invalid reviewer role');
      }
    }
