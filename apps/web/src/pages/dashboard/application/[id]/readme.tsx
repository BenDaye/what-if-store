...
import { Page as ApplicationPage, ReadmeSectionCard } from '@/components/dashboard';
...
const Page: NextPageWithLayout<InferGetServerSidePropsType<typeof getServerSideProps>> = ({ id }) => {
  ...
  return (
    <PageContainer
      ...
      header={
        <>
          <RouterBreadcrumbs label={t('application:Nav.Readme')} />
        </>
      }
    >
      <Container>
        <ApplicationPage applicationId={id}>
          <ReadmeSectionCard defaultValues={data} />
        </ApplicationPage>
      </Container>
    </PageContainer>
  );
