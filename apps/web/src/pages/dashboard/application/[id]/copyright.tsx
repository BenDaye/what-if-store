...
import { Page as ApplicationPage, CopyrightSectionCard } from '@/components/dashboard';
...
const Page: NextPageWithLayout<InferGetServerSidePropsType<typeof getServerSideProps>> = ({ id }) => {
  ...
  return (
    <PageContainer
      ...
      header={
        <>
          <RouterBreadcrumbs label={t('application:Nav.Copyright')} />
        </>
      }
    >
      <Container>
        <ApplicationPage applicationId={id}>
          <CopyrightSectionCard defaultValues={data} />
        </ApplicationPage>
      </Container>
    </PageContainer>
  );
