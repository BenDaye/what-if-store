...
import { Page as ApplicationPage, GroupSectionCard } from '@/components/dashboard';
...
const Page: NextPageWithLayout<InferGetServerSidePropsType<typeof getServerSideProps>> = ({ id }) => {
  ...
  return (
    <PageContainer
      ...
      header={
        <>
          <RouterBreadcrumbs label={t('application:Nav.Group')} />
        </>
      }
    >
      <Container>
        <ApplicationPage applicationId={id}>
          <GroupSectionCard defaultValues={data} />
        </ApplicationPage>
      </Container>
    </PageContainer>
  );
