...
import { Page as ApplicationPage, CollectionSectionCard } from '@/components/dashboard';
...
const Page: NextPageWithLayout<InferGetServerSidePropsType<typeof getServerSideProps>> = ({ id }) => {
  ...
  return (
    <PageContainer
      ...
      header={
        <>
          <RouterBreadcrumbs label={t('application:Nav.Collection')} />
        </>
      }
    >
      <Container>
        <ApplicationPage applicationId={id}>
          <CollectionSectionCard defaultValues={data} />
        </ApplicationPage>
      </Container>
    </PageContainer>
  );
