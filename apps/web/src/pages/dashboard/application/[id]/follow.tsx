...
import { Page as ApplicationPage, FollowSectionCard } from '@/components/dashboard';
...
const Page: NextPageWithLayout<InferGetServerSidePropsType<typeof getServerSideProps>> = ({ id }) => {
  ...
  return (
    <PageContainer
      ...
      header={
        <>
          <RouterBreadcrumbs label={t('application:Nav.Follow')} />
        </>
      }
    >
      <Container>
        <ApplicationPage applicationId={id}>
          <FollowSectionCard defaultValues={data} />
        </ApplicationPage>
      </Container>
    </PageContainer>
  );
