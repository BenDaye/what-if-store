...
import { Page as ApplicationPage, CompatibilitySectionCard } from '@/components/dashboard';
...
const Page: NextPageWithLayout<InferGetServerSidePropsType<typeof getServerSideProps>> = ({ id }) => {
  ...
  return (
    <PageContainer
      ...
      header={
        <>
          <RouterBreadcrumbs label={t('application:Nav.Compatibility')} />
        </>
      }
    >
      <Container>
        <ApplicationPage applicationId={id}>
          <CompatibilitySectionCard defaultValues={data} />
        </ApplicationPage>
      </Container>
    </PageContainer>
  );
